...
3. **Ground-target hitboxes (§6/§8).** Floor targets' hittable bands topped out below the ship's z ≥ 8 clamp — drums/turrets were provably unhittable with the spec's level shots. Shots stay flat (air targets — fighters, boss core — are aimed by matching altitude, the authentic mechanic); instead all floor targets share hh 5, making them strafe-able from a low dive (z ≤ ~10.6) and solid obstacles when flying low. An interim shot-drop approach was tried and reverted: it broke air combat.
